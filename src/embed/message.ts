...
import { MusedamUser, MuseDAMID } from '@/types/musedam';
...
function initializeMessageListener() {
  ...
  if (typeof window === 'undefined') {
    console.warn(
      'Cannot initialize message listener: not in browser environment',
    );
    return;
  }
        ...
        if (!message.result || !message.result.success) {
          ...
          if (message.result.code === 'cancelled') {
            ...
            pendingPromise.reject(
              new Error(message.result?.message ?? 'Unknown error'),
            );
          }
        } else {
...
if (typeof window !== 'undefined') {
  ...
  if (
    ...
    window.addEventListener('DOMContentLoaded', autoNotifyTeamSettings, {
      once: true,
    } as any);
  }
}
...
// 处理来自父项目的Action事件
function handleParentMessageAction(
  action: string,
  args: any,
  dispatchId?: string,
) {
...
type ActionMap = {
  ...
  getOrgUsers: {
    ...
    result: BaseActionResult<{ members?: MusedamUser[] }>;
  };
  'rename-smart-tag': {
    args: {
      tagId: number;
...
type ExtractSuccessData<T> = T extends { success: true; data: infer U }
  ? U
  : never;
...
export function dispatchMuseDAMClientAction<T extends keyof ActionMap>(
  ...
  if (typeof window === 'undefined') {
    return Promise.reject(
      new Error(
        'dispatchMuseDAMClientAction can only be used in browser environment',
      ),
    );
  }
  ...
  if (!window.parent || window.parent === window) {
    return Promise.reject(
      new Error('No parent window available for communication'),
    );
  }
  ...
  // 创建 Promise 并添加到队列
  const promise = new Promise<ExtractSuccessData<ActionMap[T]['result']>>(
    (resolve, reject) => {
...
export function dispatchMuseDAMClientActionResult(
  ...
  if (typeof window === 'undefined') {
    console.warn(
      'dispatchMuseDAMClientActionResult can only be used in browser environment',
    );
    return;
  }
...
export async function notifyTeamSettingsReady(): Promise<void> {
  ...
  if (typeof window === 'undefined') {
    console.warn(
      'Cannot notify team settings ready: not in browser environment',
    );
    return;
  }
  ...
  if (!window.parent || window.parent === window) {
    console.warn(
      'Cannot notify team settings ready: no parent window available',
    );
    return;
  }
  ...
  try {
    ...
    try {
      ...
      console.error(
        'Failed to send fallback team settings ready notification:',
        fallbackError,
      );
    }
  }
